...
import com.airduck.exception.AirduckException;
...
public class AirduckUtils {
    ...
    public static void checkObject(Object object) {
        ...
        if (!_class.isAnnotationPresent(Table.class))
            throw new AirduckException("{}实体对象需要添加@Table注解，否则不允许操作。", _class.getName());
    ...
    public static String getTableName(Class<?> source) {
        ...
        throw new AirduckException("未获取到@Table注解，请检查{}实体类是否添加了@Table注解", source.getName());
    }
